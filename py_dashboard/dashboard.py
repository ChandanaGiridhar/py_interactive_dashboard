...
st.title(":chart_with_upwards_trend: SuperStore Data Visualizations")
...
if fl is not None:
    ...
    # os.chdir(r"C:\Users\chand\Desktop\py_dashboard")
    df = pd.read_csv("Superstore.csv", encoding="ISO-8859-1")

col1, col2 = st.columns((2))
df["Order Date"] = pd.to_datetime(df["Order Date"])

# Giving start date and End date filter
...
# Choose your region filter - dropdown
...
# Choose your State filter - dropdown
...
# Create tree map based on region, category, and sub-category
st.subheader("Group view of Sales using Tree Map")
...
with chart1:
    st.subheader('Segment Chart sales')
    fig = px.pie(filtered_df, values="Sales", names="Segment", template="plotly_dark")
    fig.update_traces(text=filtered_df["Segment"], textposition="inside")
    st.plotly_chart(fig, use_container_width=True)

with chart2:
    st.subheader('Category Chart sales')
...
with st.expander("Summary_Table"):
    df_sample = df[0:5][["Region", "State", "City", "Profit", "Quantity"]]
